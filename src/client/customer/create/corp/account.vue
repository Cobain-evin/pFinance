<template>
  <div>
    ...
    <div class="btn-box">
      <router-link :to="'/customer/create/corp/file/' + customerId" class="btn small light">上一步</router-link>
      <a class="btn small" href="javascript:;" @click="updateAccountsInfo">下一步</a>
    </div>
    ...
    methods: {
      ...
      getCusBaseInfo(){
        if(this.customerId){
          ...
          .then(res => {
            this.customerName = res.returnData.CusComBase.cus_name
          })
        }
      },
          ...
          if(item.isNew){
            ...
            .then(res => {
              if(res.returnCode == '000'){
                n++
                if(n >= l){
                  This.$router.push('/workflow/success/customer')
                }
                else{
                  updateAccount()
                }
            ...
            param.or_recv_acct_no = item.or_recv_acct_no
