<template>
  <div>
    <!-- 个体工商户 -->
    <div v-if="isOwnedBusi">
      <div class="no-data">个体工商户不需要录入股权信息</div>
      <div class="btn-box">
        <router-link :to="'/customer/create/corp/contact/' + customerId" class="btn small light">上一步</router-link>
        <router-link :to="'/customer/create/corp/file/' + customerId" class="btn small">下一步</router-link>
      </div>
    </div>
    ...
    <div v-if="!isOwnedBusi">
      ...
      <div class="btn-box">
        <router-link :to="'/customer/create/corp/contact/' + customerId" class="btn small light">上一步</router-link>
        <a class="btn small" href="javascript:;" @click="updateStocksInfo">下一步</a>
      </div>
    </div>
    <select-customer 
          ...
          if(item.isNew){
            ...
            .then(res => {
              if(res.returnCode == '000'){
                n++
                if(n >= l){
                  This.$router.push('/customer/create/corp/file/' + This.customerId)
                }
                else{
                  updateStock()
                }
